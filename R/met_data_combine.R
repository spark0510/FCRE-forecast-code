met_data_bind <- function(realtime_file,
                     qaqc_file,
                     cleaned_met_file,
                     input_file_tz,
                     nldas = NULL,
  ...
  if(!is.na(qaqc_file)){
    ...
    TIMESTAMP_in <- lubridate::force_tz(d1$DateTime, tzone = input_file_tz)
    ...
    TIMESTAMP_in <- lubridate::force_tz(d2_time_update$DateTime, tzone = input_file_tz)
    d2_time_update$TIMESTAMP <- lubridate::with_tz(TIMESTAMP_in,tz = "UTC")
    ...
    d1 <- data.frame(time = d1$TIMESTAMP, ShortWave = d1$ShortwaveRadiationUp_Average_W_m2, LongWave = d1$InfraredRadiationUp_Average_W_m2, AirTemp = d1$AirTemp_C_Average, RelHum = d1$RH_percent, WindSpeed = d1$WindSpeed_Average_m_s, Rain = d1$Rain_Total_mm, pressure = d1$BP_Average_kPa)
    d2_update <- data.frame(time = d2_time_update$TIMESTAMP, ShortWave = d2_time_update$ShortwaveRadiationUp_Average_W_m2, LongWave = d2_time_update$InfraredRadiationUp_Average_W_m2, AirTemp = d2_time_update$AirTemp_C_Average, RelHum = d2_time_update$RH_percent, WindSpeed = d2_time_update$WindSpeed_Average_m_s, Rain = d2_time_update$Rain_Total_mm, pressure = d2_time_update$BP_Average_kPa)

    d1 <- d1[which(d1$time > d2_update$time[nrow(d2_update)] | d1$time < d2_update$time[1]), ]

    #d3 <- d3[which(d3$TIMESTAMP < d2$TIMESTAMP[1])]

    d <- rbind(d2_update, d1)
